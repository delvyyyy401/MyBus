...
class Pending_mybus extends CI_Controller {
	...
	public function viewpending($id=''){
		$cek = $this->input->get('order').$id;
	 	...
	 	if ($sqlcek) {
			...
			$this->load->view('backend/view_pending',$data);
	 	}else{
	...
	public function viewpending_institusi($id=''){
		$cek = $this->input->get('order').$id;
	 	$sqlcek = $this->db->query("SELECT * FROM tbl_order_mybus LEFT JOIN tbl_jadwal_mybus on tbl_order_mybus.kd_jadwal = tbl_jadwal_mybus.kd_jadwal WHERE tbl_order_mybus.jumlah_kursi_institusi > 4  AND kd_order ='".$cek."'")->result_array();
	 	if ($sqlcek) {
	 		$data['tiket'] = $sqlcek;
			$data['title'] = "View Order";
			$this->load->view('backend/view_pending',$data);
	...
	public function inserttiket_bypending($value=''){
		...
		redirect('backend/pending_mybus');
	}
