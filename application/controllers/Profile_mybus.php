...
class Profile_mybus extends CI_Controller {
	...
	public function profilesaya($id=''){
		$data['profile'] = $this->db->query("SELECT * FROM tbl_pelanggan_mybus WHERE kd_pelanggan LIKE '".$id."'")->row_array();
		// die(print_r($data));
	...
	public function editprofile($id=''){
		...
		redirect('profile_mybus/profilesaya/'.$id);
	}
	public function tiketsaya($id=''){
		$this->getsecurity();
		$data['tiket'] = $this->db->query("SELECT * FROM tbl_order_mybus WHERE kd_pelanggan ='".$id."' group by kd_order")->result_array();
		// die(print_r($data));
		$this->load->view('frontend/tiketmu',$data);
	}
	public function changepassword($id=''){
		$this->load->library('form_validation');
		$pelanggan = $this->db->query("SELECT password_pelanggan FROM tbl_pelanggan_mybus where kd_pelanggan ='".$id."'")->row_array();
		...
		if ($this->form_validation->run() == false) {
			...
			if (!password_verify($currentpassword, $pelanggan['password_pelanggan'])) {
				...
				redirect('profile_mybus/changepassword');
